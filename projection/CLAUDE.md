...
- **14–30 days out:** blend accumulation projection with confidence tiers (WBH × WBH-to-start rate + VIP+Priority × pooled rate).
- **Under 14 days:** confidence tiers dominate. WBH × show rate is the floor; VIP+Priority × pooled rate is added for the mid. Published low/high are re-sized around the mid from observed error.
- **VIP and P-xx (P-FA/P-VA/P-Acc/P-Adm) are one pooled tier** (`VIP+Priority` in `baselines/confidence_tier_rates.csv`), per Clanton 2026-09-18: they convert alike and neither has the sample to stand alone. **Count students, not flags:** many students carry VIP and a P-xx flag (UDT570 on 9/18: 66 flags, 37 students), so the pool is `vip_priority_count` (VIP or any P-xx, and not WBH -- each student once, in their highest tier; `ingest.vip_priority_students`). Calibration reads only `vip_priority_at_start` / `vip_priority_that_started` in `cohort_actuals.csv`; the per-flag vip/priority columns overlap and must never be summed. Do not split the tier back out.
- **Tier calibration is horizon-matched for VIP+Priority only.** `tier_observations.py` records every tagged student in every EnrollList 0-29d before start (`*_obs_*` columns); calibration uses those for VIP+Priority because at start (~3d out) its eventual starters have already become WBH. **WBH deliberately stays at-start:** its window rate (~0.79) is truer, but the model under-projects near start (untagged and late-enrolling starters have no term) and the higher at-start WBH rate partly covers that -- a 566-569 replay on the window worsened the backtest. Flip WBH when the missing-starters term is built, not before.
