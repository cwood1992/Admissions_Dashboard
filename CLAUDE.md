...
The dashboard fetches CSV files at runtime and requires a local server:
...
- **`index.html`** — The entire dashboard: all HTML, CSS, and JavaScript in one self-contained file. No build step.
...
1. **CONFIG** (~line 380) — Hardcoded constants: `COLORS`, `EXCLUDED_REPS`, `MIN_REP_THRESHOLD`, `REP_START_COHORT`, `REP_TARGETS`, `REP_START_TARGETS`, `PROGRAM_TARGET`.
2. **STATE** (~line 428) — Global mutable state: `rawData`, `leadsData`, `availableYears`, `selectedYears`, `selectedRepsFilter`, `activeReps`, `repColorMap`, `processedData`, `charts`, `currentChartType`, `leadsPeriod`.
3. **UTILITIES** (~line 444) — `extractCohortNumber`, `destroyChart`, `getRepColor`, `rollingAvg`, `isAllReps`, `getSingleRep`, `getFilteredReps`.
4. **LOAD DATA** (~line 481) — `loadData()`: fetches `summary_index.json`, then each year CSV, then `leads.csv`. Falls back to `summary.csv` if no index.
5. **CONTROLS INIT** (~line 556) — `initControls()`: sets up year checkboxes, cohort range sliders, program filter, rep chip listener, chart type buttons.
6. **DATA PROCESSING** (~line 617) — `processData()`: filters `rawData` by active years/cohort range, calls `detectActiveReps()`, builds `processedData` object keyed by program.
7. **Chart update functions** — One function per chart section, all called from `updateDashboard()`.
...
- `getFilteredReps()` — returns the subset of `activeReps` matching `selectedRepsFilter` (or all `activeReps` if none selected)
...
`selectedYears` (a `Set`) controls which year files are active. Year checkboxes are shown only when multiple years are loaded. The YOY chart and Cross-Year chart use `processedData._yearData` which is keyed by program and then year.

### Cohort Identification

Cohort names follow the pattern `NDT560`, `UDT562`, `NDT562NC`. `extractCohortNumber()` pulls the numeric portion for sorting and range filtering. `extract_program()` in `process_ccs.py` maps name patterns to program types (NDT, UDT, NDT-NC, UDT-NC).
