...
├─ ledger/                       ← canonical join layer (cash foundation)
│  ├─ build_ledger.py            ← joins both engines' OUTPUTS by cohort code
│  ├─ build_cashflow.py          ← cashflow projection: FA xlsx + schedule + ledger → cash/data/
│  ├─ cohort_ledger.json         ← generated: per-cohort lifecycle + booked/projected revenue
│  └─ cohort_ledger.schema.json
└─ cash/                         ← CASH engine (cash-flow timing dashboard)
   ├─ index.html, assets/        ← dark dashboard, Chart.js, reads data/cashflow.js
   └─ data/cashflow.json|.js     ← generated by ledger/build_cashflow.py (aggregates only)
```
...
python ledger\build_cashflow.py                            # 4. cash-flow projection (reads FA OneDrive xlsx)
python -m http.server 8000                                 # 5. serve from ROOT
# open http://localhost:8000/shell/index.html
```

`ledger/build_ledger.py` runs under ambient `python` (needs pandas); it imports `program_for_cohort` / `position_for_cohort` / `REVENUE_PER_START` from `projection/scripts/`.

### Cash engine (cash/ + ledger/build_cashflow.py)
...
- The cash low/high scenario stacks every in-flight cohort's `proj_low` (or `proj_high`) at once. The projection engine's FY ranges combine cohorts as independent errors instead (`projection/scripts/error_bands.py`), so the cash low/high spread is wider than the FY revenue range by design of the scenario, not by error.
